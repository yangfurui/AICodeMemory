...
Claude Code 的会话记录 **30 天后被自动清理**,而且每个新会话都是失忆重启——上周定的方案、踩过的坑、说过的原话,全部找不回。claudeMemory 把它们变成一个可语义检索的本地库:
...
```bash
git clone https://github.com/furuiyang/claudeMemory.git
cd claudeMemory
...
去噪(只留人/AI 对话,滤工具输出、思维链、系统噪音)
 ...
 → SQLite(原文 + 向量,~/.claudememory/memory.sqlite3)
    ▲
    │  cmem search
