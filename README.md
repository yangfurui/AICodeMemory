...
cmem setup               # 自动注册到已安装的 Claude Code / Codex
```
...
推荐用一条命令把 AICodeMemory 注册为本地 stdio MCP server:

```bash
cmem setup
```

它会同时检测 `claude` 和 `codex` CLI,安装了哪个就为哪个添加用户级
`cmem` 配置;重复执行不会重复写入,遇到不同的同名配置会停止而不覆盖。
server 只注册 3 个只读工具:
...
自行检索并引用原话回答。
需要撤销时运行 `cmem setup --remove`,它只会移除 AICodeMemory 的 MCP 配置和
`CLAUDE.md` / `AGENTS.md` marker 区块。

隐私边界:embedding、存档和检索都在本地;但 AI 调用 MCP 或 CLI 后,返回的命中
原文会进入当前 Claude/Codex 的模型上下文,并非"所有内容永不离机"。
